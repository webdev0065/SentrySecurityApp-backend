...
router.put('/agencies/:id', async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: 'Complete all required agency details.' });
  if (
    ...
    return res
      .status(400)
      .json({
        error: 'Enter a valid email, Indian mobile number and pincode.',
      });
  if (
    !['active', 'inactive'].includes(status) ||
    (gstNumber && !/^[A-Z0-9]{15}$/.test(gstNumber))
  )
    return res.status(400).json({ error: 'Invalid status or GST number.' });
  if (
    newPassword &&
  ...
  try {
    ...
    const existing = await client.query(
      'SELECT user_id, status FROM agencies WHERE id=$1 FOR UPDATE',
      [req.params.id],
    );
    ...
    if (!['approved', 'inactive'].includes(existing.rows[0].status)) {
      await client.query('ROLLBACK');
      return res
        .status(409)
        .json({ error: 'Review this agency in the Approval tab first.' });
    }
    const userId = existing.rows[0].user_id;
    await client.query(
      'UPDATE users SET full_name=$1, email=$2, mobile_number=$3 WHERE id=$4',
      [ownerName.trim(), email.trim().toLowerCase(), mobile, userId],
    );
    if (newPassword)
    ...
    await client.query(
      'UPDATE agencies SET agency_name=$1, office_address=$2, state=$3, district=$4, city=$5, pincode=$6, gst_number=$7, status=$8 WHERE id=$9',
      [
        agencyName.trim(),
        address.trim(),
        state,
