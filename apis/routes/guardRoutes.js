  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message: 'All fields except age and allowances are required',
        });
    }
    ...
    if (!isValidDate(joiningDate)) {
      return res
        ...
        .json({
          success: false,
          message: 'joiningDate must be a valid date in YYYY-MM-DD format',
        });
    }
    if (
      Number(basicSalary) <= 0 ||
      Number(basicSalary) > 99999999.99 ||
      (allowances != null &&
        (Number(allowances) < 0 || Number(allowances) > 99999999.99))
    ) {
      return res
        .status(400)
        .json({
          success: false,
          message:
            'basicSalary must be positive and allowances cannot be negative',
        });
    }
    if (coveragePlan !== '24x7') {
      if (!shiftHours || !VALID_SHIFT_HOURS.includes(Number(shiftHours))) {
        return res
          ...
          .json({
            success: false,
            message: 'shiftHours must be 8 or 12 for day_shift/night_watch',
          });
      }
      if (!startTime || !endTime) {
        return res
          ...
          .json({
            success: false,
            message:
              'startTime and endTime are required for day_shift/night_watch',
          });
      }
    }
  ...
  try {
    ...
    if (!isValidEmail(email) || !isValidDate(joiningDate)) {
      return res
        ...
        .json({
          success: false,
          message: 'A valid email and joining date are required',
        });
    }
    ...
    if (!VALID_COVERAGE_PLANS.includes(coveragePlan)) {
      return res
        .status(400)
        .json({
          success: false,
    ...
    const guard = await Guard.update(req.params.id, req.user.id, {
      ...
      coveragePlan,
      shiftHours: coveragePlan === '24x7' ? null : Number(shiftHours),
      startTime: coveragePlan === '24x7' ? null : startTime,
      endTime: coveragePlan === '24x7' ? null : endTime,
      basicSalary,
      allowances,
      address,
      age,
...
router.delete('/guards/:id', verifyToken, async (req, res) => {
  try {
    const guard = await Guard.delete(req.params.id, req.user.id);
    if (!guard)
      return res
        .status(404)
        .json({ success: false, message: 'Guard not found' });
    return res.status(200).json({ success: true, message: 'Guard removed' });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ success: false, message: 'Server error' });
  }
  ...
  try {
    ...
    if (!VALID_STATUSES.includes(status)) {
      return res
        ...
        .json({
          success: false,
          message: 'status must be on_duty or off_duty',
        });
    }
...
router.patch('/guards/:id/location', verifyToken, async (req, res) => {
  try {
    ...
    const guard = await Guard.updateLocation(
      req.params.id,
      req.user.id,
      latitude,
      longitude,
