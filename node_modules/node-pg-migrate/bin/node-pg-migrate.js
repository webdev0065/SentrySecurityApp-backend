#!/usr/bin/env node
import { Migration, PG_MIGRATE_LOCK_ID, jiti, runner } from "node-pg-migrate";
...
const parser = yargs(process.argv.slice(2)).usage("Usage: $0 [up|down|create|redo] [migrationName] [options]").options({
	...
	[advisoryLockModeArg]: {
		default: "fail",
		describe: "Controls behavior when the migration advisory lock is already held by another process. Use \"fail\" to throw immediately or \"wait\" to block until the lock becomes available",
		choices: ["fail", "wait"],
		type: "string"
	},
	[tsconfigPathsArg]: {
		defaultDescription: "false",
		describe: "Enable jiti's tsconfig paths resolution when loading TS/JS migration files. Pass `true` to auto-discover tsconfig.json, or a path to a specific tsconfig.json",
		type: "string"
	}
...
if (dotenv) {
	...
	if (dotenvExpand && dotenvExpand.expand) dotenvExpand.expand(myEnv);
}
...
let ADVISORY_LOCK_MODE = argv[advisoryLockModeArg];
let TSCONFIG_PATHS = parseTsconfigPaths(argv[tsconfigPathsArg]);
function applyIf(arg, key, obj, condition) {
	if (arg !== void 0 && !(key in obj)) return arg;
	const val = obj[key];
...
function isBooleanOrString(val) {
...
*/
function parseTsconfigPaths(val) {
...
function readJson(json) {
	if (typeof json === "object" && json !== null) {
		SCHEMA = applyIf(SCHEMA, schemaArg, json, (val) => Array.isArray(val) || isString(val) && val.length > 0);
		...
		MIGRATIONS_SCHEMA = applyIf(MIGRATIONS_SCHEMA, migrationsSchemaArg, json, isString);
		CREATE_MIGRATIONS_SCHEMA = applyIf(CREATE_MIGRATIONS_SCHEMA, createMigrationsSchemaArg, json, isBoolean);
		MIGRATIONS_TABLE = applyIf(MIGRATIONS_TABLE, migrationsTableArg, json, isString);
		MIGRATIONS_FILE_LANGUAGE = applyIf(MIGRATIONS_FILE_LANGUAGE, migrationFileLanguageArg, json, (val) => val === "js" || val === "ts" || val === "sql" || val === "cjs" || val === "mjs" || val === "cts" || val === "mts");
		MIGRATIONS_FILENAME_FORMAT = applyIf(MIGRATIONS_FILENAME_FORMAT, migrationFilenameFormatArg, json, (val) => val === "timestamp" || val === "utc" || val === "index");
		TEMPLATE_FILE_NAME = applyIf(TEMPLATE_FILE_NAME, templateFileNameArg, json, isString);
		...
		TSCONFIG_PATHS = applyIf(TSCONFIG_PATHS, tsconfigPathsArg, json, isBooleanOrString);
		DB_CONNECTION = applyIf(DB_CONNECTION, databaseUrlVarArg, json, (val) => typeof val === "string" || typeof val === "object");
		if ("url" in json && json.url) DB_CONNECTION ??= json.url;
		else if (isClientConfig(json) && !DB_CONNECTION) DB_CONNECTION = {
			user: json.user,
		...
		ADVISORY_LOCK_MODE = applyIf(ADVISORY_LOCK_MODE, advisoryLockModeArg, json, (val) => val === "fail" || val === "wait");
	} else DB_CONNECTION ??= json;
}
...
if (config?.has(argv[configValueArg])) readJson(config.get(argv[configValueArg]));
process.env.SUPPRESS_NO_CONFIG_WARNING = oldSuppressWarning;
const configFileName = argv[configFileArg];
if (configFileName) {
	const configModule = await jiti.import(resolve(configFileName));
	let json;
	if (configModule && typeof configModule === "object" && "default" in configModule) json = configModule.default;
	else json = configModule;
	const section = argv[configValueArg];
	if (json && typeof json === "object" && section in json) readJson(json[section]);
	else readJson(json);
}
...
if (action === "create") {
	...
	if (!DB_CONNECTION) {
		...
		if (!process.env[argv[databaseUrlVarArg]] && (!process.env.PGHOST || !cp.user || !cp.database)) {
			console.error(`The ${argv[databaseUrlVarArg]} environment variable is not set or incomplete connection parameters are provided.`);
			process.exit(1);
		}
		DB_CONNECTION = cp;
	}
	...
	if (noLock) console.log("no lock");
	const upDownArg = argv._.length > 0 ? argv._[0] : null;
	let numMigrations;
	let migrationName;
	if (upDownArg !== null) {
		const parsedUpDownArg = Number.parseInt(`${upDownArg}`, 10);
		if (parsedUpDownArg == upDownArg) numMigrations = parsedUpDownArg;
		else migrationName = argv._.join("-").replace(/_ /g, "-");
	}
	...
	const options = (direction, _count, _timestamp) => {
		...
		return {
			...
			databaseUrl: {
				...databaseUrl,
				...typeof rejectUnauthorized === "boolean" ? { ssl: {
					...databaseUrl.ssl,
					rejectUnauthorized
				} } : void 0
			},
	...
	(action === "redo" ? runner(options("down")).then(() => runner(options("up", Number.POSITIVE_INFINITY, false))) : runner(options(action))).then(() => {
